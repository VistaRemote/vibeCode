...
if (!metaRoot) {
  console.error('â?Run this from Meta-Repo root or a sub-repo with ../vista-remote tooling.');
  process.exit(1);
}
...
console.log('ðŸ›  VistaRemote single-repo setup\n');
...
if (existsSync(join(metaRoot, 'shared'))) {
  ...
  try {
    ...
    console.warn('âš ï¸  shared build failed â€?check Node >= 24.11');
  }
}
