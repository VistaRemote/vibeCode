...
| **M1 MVP-A** | MySQL + JWT + 真实鉴权 | 🟡 | ADM-0 Admin JWT 已落地；User 持久化 ADM-1 |
| **M3 MVP-C** | BullMQ + AI 摘要 | ⬜ | 队列任务可追踪 |
| **M4 MVP-D** | 录制 + MinIO | 🟡 | Admin 录制元数据 ADM-0；上传/MinIO ADM-3 |
...
> 详规：[plan/admin-platform-roadmap.md](./plan/admin-platform-roadmap.md) · [spec/web-admin-spec.md](./spec/web-admin-spec.md)

| 能力 | 说明 | 版本 |
| :--- | :--- | :--- |
| **Admin 登录 + JWT** | `POST /admin/auth/login`；RBAC 守卫 | **ADM-0** 🟢 |
| **用户/订单/套餐** | 内存 store；改套餐写审计 | ADM-0 |
| **录制元数据** | 列表 + 回放 URL 桩 | ADM-0；MinIO ADM-3 |
| **活跃会话** | Room 列表、强制断开 | ADM-0 |
| **MySQL 用户** | 注册/登录、Device 持久化 | ADM-1 / M1 |
| **组织/安全/AI** | Enterprise 策略与事件 | ADM-5 |
